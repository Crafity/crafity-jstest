...
var JSLINT = require('./~jslint.module');
...
(function (argv) {
  ...
  if (!argv[0]) {
    console.log("Usage: jslint.js file.js [realfilename.js]");
    process.exit(1);
  }
  ...
  fs.readFile(fileToParse, function (err, data) {
    if (err || !data) {
      console.log("jslint: Couldn't open file '" + fileToParse + "'.");
      process.exit(1);
    }
    ...
    if (!JSLINT(data.toString(), defaults)) {
      ...
      console.log("jslint: No problems found in " + fileToDisplay);
