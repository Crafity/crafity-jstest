...
function Context(name) {
  ...
  this.run = function run(tests, callback) {
    ...
    if (name) {
      formatter.format(null, "Executing '" + name + "' with " + Object.keys(tests).length + " test(s)");
    } else {
      formatter.format(null, "Executing " + Object.keys(tests).length + " test(s)");
    }
...
var jstestModule = {};
jstestModule.createContext = function createContext(name) {
...
jstestModule.version = "0.1.0";
jstestModule.fullname = "crafity-jstest";
jstestModule.assert = assert;
